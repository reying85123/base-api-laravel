...
class CreatePermissionTables extends Migration
{
    ...
    public function up()
    {
        ...
        Schema::create($tableNames['roles'], function (Blueprint $table) use ($teams, $columnNames) {
            ...
            $table->string('name');       // For MySQL 8.0 use string('name', 125);
            $table->string('guard_name'); // For MySQL 8.0 use string('guard_name', 125);
            $table->boolean('is_superadmin')->default(0)->comment('是否為超級管理者群組');
        ...
        \DB::statement("ALTER TABLE `permissions` comment '功能權限列表'");
        \DB::statement("ALTER TABLE `roles` comment '權限群組'");
        \DB::statement("ALTER TABLE `role_has_permissions` comment '權限授權群組'");
        \DB::statement("ALTER TABLE `model_has_roles` comment '群組權限授權人員'");
        \DB::statement("ALTER TABLE `model_has_permissions` comment '權限授權人員'");
    }
